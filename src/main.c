...
int init_cm_hal_init(void)
{
    int ret = 0;
    ret = cm_hal_InitDB();
    if (ret == 0)
    {
        UT_LOG_DEBUG("cm_hal_InitDB returned success");
        return 0;
    }
    else
    {
        UT_LOG_DEBUG("cm_hal_InitDB returned failure");
        UT_FAIL("cm_hal_InitDB initialization failed");
        return -1;
    }
}
